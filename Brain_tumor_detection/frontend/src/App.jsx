...
const formatMetrics = (metrics) => {
  ...
  let metricStrings = Object.entries(metrics).map(([key, value]) => {
    if (typeof value === 'object') {
      const perClass = Object.entries(value)
        .map(([label, val]) => `  - ${label}: ${JSON.stringify(val)}`)
        .join('\n')
      return `${key}:\n${perClass}`
    }
    return `${key}: ${value}`
...
async function sendToFlowise(content) {
  try {
    ...
    if (!flowiseRes.ok) {
      ...
      throw new Error(
        `Flowise API error: ${flowiseRes.status} ${
          flowiseRes.statusText
  ...
  async function createNewChat(topic, content) {
    ...
    try {
      ...
      const res = await fetch(
        'https://4xrw8qp1-8000.asse.devtunnels.ms/submit_case',
        {
      ...
      const info = is2D
        ? `Tumor type from predict : ${data.tumor_type_predict} and testIndication ${content.testIndication}`
      ...
      if (is2D && data.images?.length) {
        conversation.push({
          sender: 'ai',
          text: '🖼️ Detected 2D images from your file.',
        })
      }
